...
# =============================================================================
# cf_bypass.sh
# Cloudflare bypass PORT NGUYÊN VẸN từ:
#   Revanced-And-Revanced-Extended-Non-Root (src/build/utils.sh)
#   -> engine: FlareSolverr (8191) chính, CloudflareBypassForScraping (8000) dự phòng
#
# Chiến lược tích hợp (bền hơn port selector pup):
#   Nguồn mới "apkmirror_cf" TÁI DÙNG các hàm apkmirror ĐÃ KIỂM CHỨNG của
#   revanced-magisk-module (get_apkmirror_resp / get_apkmirror_vers /
#   dl_apkmirror, parse bằng htmlq), nhưng tạm thời THAY 'req' bằng phiên bản
#   đi qua Cloudflare bypass trong đúng phạm vi các lời gọi đó.
#
#   -> Không lệ thuộc selector 'pup' dễ vỡ.
#   -> Không có HTML state toàn cục leak giữa các app (lỗi tải nhầm APK trước đây).
#
# Trong config.toml dùng khóa:  apkmirror_cf-dlurl = "<trang app apkmirror>"
# =============================================================================

# --------------------------------------------------------------- deps -------
# user_agent: giữ y hệt repo1 (tự dò UA Firefox mới nhất, fallback cứng).
...
# =========================== BYPASS CORE (repo1) =============================
# --- _fs_get: FlareSolverr (8191). GIỮ NGUYÊN từ repo1. ---------------------
_fs_get() {
	local url=$1
	local max_retries=3
	local attempt
...
# --- _cfb_get: CloudflareBypassForScraping (8000). GIỮ NGUYÊN từ repo1. ------
_cfb_get() {
	local url=$1
	local max_retries=3
	local attempt
...
# ===================== req override đi qua bypass ===========================
# cf_req: tương thích chữ ký req() của repo2:  req <url> <output|->
#   - output == "-"  : trả HTML (đọc qua bypass, tránh Cloudflare 403).
#   - output != "-"  : tải file nhị phân bằng cookie/UA đã lấy từ bypass.
# Repo2 truyền output là ĐƯỜNG DẪN đầy đủ; ta tôn trọng đúng đường dẫn đó.
cf_req() {
	local url=$1 op=$2
	if [ "$op" = "-" ]; then
		local html=""
...
# ===================== SHIMS: nguồn "apkmirror_cf" ==========================
# Bọc từng hàm apkmirror của repo2, tạm override 'req' -> 'cf_req' trong đúng
# phạm vi lời gọi rồi khôi phục. Dùng lại toàn bộ logic parse (htmlq) đã kiểm
# chứng của repo2 mà mọi request đi qua Cloudflare bypass.
_cf_with_req() {
	local _orig_req
